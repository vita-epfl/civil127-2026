import os
import pygame
from lab6.marmotte_sweeper.logic import Cell, MarmotteSweeper, RealRand
...
class MarmotteSweeperUI:
    ...
    def __init__(self, width: int, height: int, count_marmottes: int, cell_size: int):
        ...
        self.images: dict[Cell, pygame.Surface] = {}
        self.number_images: dict[int, pygame.Surface] = {}
        for i in range(9):
            self.number_images[i] = pygame.image.load(
                os.path.join("assets", str(i) + ".png")
            )
        ...
        for k, v in self.number_images.items():
            self.number_images[k] = pygame.transform.smoothscale(
                v, (self.cell_size, self.cell_size)
            )
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                if cell == Cell.REVEALED:
                    image = self.number_images[self.marmotte_sweeper.count(x, y)]
                else:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.button == 3:
                        ...
                        # pick one.
                        # self.marmotte_sweeper.reveal((x, y))
                        # self.marmotte_sweeper.reveal_recursive(x, y)
                        self.marmotte_sweeper.reveal_iterative({(x, y)})
